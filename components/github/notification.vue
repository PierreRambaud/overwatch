<template>
    <li class="list-group-item js-notification js-navigation-item unread issue-notification" aria-selected="false">
        <a class="list-group-item-link" :href="issueUrl" :title="notification.subject.title">
            ...
            <span class="list-group-item-time">
                <timeago :datetime="notification.updated_at" :auto-update="30"/>
            </span>
        </a>
    </li>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      isPullRequest() {
        return this.notification.subject.type === constant.NOTIFICATION_TYPE.PR;
      },
      isIssue() {
        return this.notification.subject.type === constant.NOTIFICATION_TYPE.ISSUE;
      },
      iconState() {
      ...
      issueUrl() {
        let url = this.issue.html_url;
        switch (this.notification.reason) {
          case constant.NOTIFICATION_REASON.MENTION:
            break;
          case constant.NOTIFICATION_REASON.ASSIGN:
            url += '';
            break;
          case constant.NOTIFICATION_REASON.AUTHOR:
            break;
          case constant.NOTIFICATION_REASON.COMMENT:
            break;
          case constant.NOTIFICATION_REASON.INVITATION:
            break;
          case constant.NOTIFICATION_REASON.MANUAL:
            break;
          case constant.NOTIFICATION_REASON.STATE:
            break;
          case constant.NOTIFICATION_REASON.SUBSCRIBED:
            break;
          case constant.NOTIFICATION_REASON.TEAM_MENTION:
            break;
          default:
            break;
        }
    ...
    methods: {
      getType() {
        return this.isIssue ? '/api/github/issue' : '/api/github/pulls';
      },
    },
    async mounted() {
